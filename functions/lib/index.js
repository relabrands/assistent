...
async function syncNotionLogic({ startDate = "2026-08-01", cleanBefore = true } = {}) {
    ...
    const tokenize = (s) => s.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").split(/[^a-z0-9]+/).filter(Boolean);
    const findMatchingClient = (title, dbId) => {
        // 1. Exact notion_database_id match has absolute priority
        const idMatch = crmClients.find(c => c.notion_database_id && c.notion_database_id.replace(/-/g, "").toLowerCase() === dbId.replace(/-/g, "").toLowerCase());
        if (idMatch)
            return idMatch;
        const normTitle = normalizeName(title);
        const titleTokens = tokenize(title);
        // 2. Exact brand_name or name match
        const exactMatch = crmClients.find(c => (c.brand_name && normalizeName(c.brand_name) === normTitle) ||
        ...
        return crmClients.find(c => {
            ...
            if (brandTokens.length > 0 && brandTokens.every(t => titleTokens.includes(t)))
                return true;
            if (nameTokens.length > 0 && nameTokens.every(t => titleTokens.includes(t)))
                return true;
            return false;
        });
    };
    ...
    for (const dbItem of databases) {
        ...
        for (const page of pages) {
            ...
            if (postDate && postDate >= startDate && postDate <= todayStr && !isPosted && !isDiscarded) {
                ...
                if (!existingNotionIds.has(page.id)) {
                    ...
                    existingTasksMap.set(page.id, newDocRef);
        ...
        const matchingQuotaDocs = [];
        const matchingContentPlanningTasks = [];
        tasksSnap.docs.forEach(doc => {
            const data = doc.data();
            const pId = data.notion_page_id || "";
            const dClient = (data.client || "").toLowerCase();
            const dTitle = (data.title || "").toLowerCase();
            const isThisClient = dClient === clientName.toLowerCase() ||
        ...
        if (dbMonthPostsCount >= clientQuota) {
            ...
            for (const tDoc of tasksSnap.docs) {
                ...
                const updatedSubtasks = data.subtasks.map((st) => {
                    ...
                    const matches = stTitle.includes(clientName.toLowerCase()) ||
            ...
            if (existingQuotaDoc) {
                ...
                existingTasksMap.set(quotaDedupeKey, newDoc);
